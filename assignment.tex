...
\begin{document}
	...
	%\setlist[enumerate,1]{before=\renewcommand\theequation{\theenumi.\arabic{equation}}
		...
		\author{SRIJAN (BT21BTECH11007)% <-this % stops a space
		}	
		%\title{
...
\textbf{ Problem: } $ PQR $ is a triangle.$  S $ is a point on the side $ QR $ of $ PQR $ such that $\angle{PSR} = \angle{QPR}$.\\
Given $  QP = 8 cm$,$  PR = 6 cm $ and $ SR = 3 cm $\\
\begin{enumerate}
	\item  Prove $\Delta{PQR}$ $\sim$ $\Delta{SPR}$\\
	\item Find the length of sides $  QR $ and $ PS $\\
	\item $\dfrac{area(\Delta{PQR})}{area (\Delta{SPR})}$
\end{enumerate}
...
\begin{enumerate}
	...
	\item $\dfrac{area(\Delta{PQR})}{area (\Delta{SPR})} = \dfrac{6^2}{3^2}=\dfrac{4}{1}$
